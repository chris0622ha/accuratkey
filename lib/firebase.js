...
export async function getProfiles(uid) {
  ...
  return snap.docs.map(d => ({ id: d.id, ...d.data() })).filter(s => !s.removed).slice(0, 20);
}
...
export async function saveSession(uid, profileId, session) {
  ...
  // Daily Keys cap — max 500 per day to prevent farming
  const todayET = getETDateStr();
  const dailyReset = p.dailyKeysDate !== todayET;
  const dailyKeysToday = dailyReset ? 0 : (p.dailyKeysEarned || 0);
  const DAILY_CAP = 500;
  const actualEarned = Math.max(0, Math.min(keysEarned, DAILY_CAP - dailyKeysToday));
  const newDailyKeys = dailyKeysToday + actualEarned;
...
// ─── Maintenance mode ─────────────────────────────────────────────────────────
export async function setMaintenanceMode(enabled, message = '', triggers = {owner:true,admins:false,users:false}) {
  await setDoc(doc(db, 'config', 'maintenance'), { enabled, message, triggers, updatedAt: serverTimestamp() });
}
...
// ─── Add keys (admin) ─────────────────────────────────────────────────────────
export async function adminSetKeys(targetUid, profileId, amount) {
  const ref = doc(db, 'accounts', targetUid, 'profiles', profileId);
  await updateDoc(ref, { keys: amount });
  return amount;
}

export async function adminSetTrials(targetUid, profileId, count) {
...
const MAX_LEGIT_WPM = 250; // World record is ~212 WPM
...
export async function submitDailyScore(uid, username, avatar, { wpm, accuracy }) {
  ...
  if (suspicious) {
    await addDoc(collection(db, 'flaggedScores'), {
      uid, username, avatar, wpm, accuracy, date: today,
      reason: wpm > MAX_LEGIT_WPM ? `WPM ${wpm} exceeds maximum (${MAX_LEGIT_WPM})` : `Suspicious accuracy ${accuracy}%`,
      status: 'pending',
      createdAt: serverTimestamp(),
    });
  }
...
export async function removeLeaderboardScore(uid, date) {
  const ref = doc(db, 'dailyScores', date || getETDateStr(), 'scores', uid);
  await updateDoc(ref, { removed: true, removedAt: serverTimestamp() });
}

export async function restoreLeaderboardScore(uid, date) {
  const ref = doc(db, 'dailyScores', date || getETDateStr(), 'scores', uid);
  await updateDoc(ref, { removed: false, restored: true, restoredAt: serverTimestamp() });
}

export async function requestScoreRestore(uid, date, reason) {
  await addDoc(collection(db, 'restoreRequests'), {
    uid, date, reason, status: 'pending', createdAt: serverTimestamp(),
  });
}

export async function getFlaggedScores() {
  const q = query(collection(db, 'flaggedScores'), where('status', '==', 'pending'), orderBy('createdAt', 'desc'), limit(50));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function getDailyLeaderboard() {
  const today = getETDateStr();
  const q = query(collection(db, 'dailyScores', today, 'scores'), orderBy('wpm', 'desc'), limit(50));
  const snap = await getDocs(q);
  // Filter removed/suspicious scores client-side
  return snap.docs.map(d => d.data());
}
...
// ─── Admin: read feedback ─────────────────────────────────────────────────────
export async function getAdminFeedback(limitN = 50) {
  ...
  for (const acc of accounts.docs) {
    try {
      const snap = await getDocs(query(
        collection(db, 'accounts', acc.id, 'feedback'),
...
export async function dismissFeedback(uid, docId) {
  await deleteDoc(doc(db, 'accounts', uid, 'feedback', docId));
}
...
export async function getAdminBirthdayRequests() {
  const accs = await getDocs(collection(db, 'accounts'));
  const all = [];
  for (const acc of accs.docs) {
    try {
      const snap = await getDocs(collection(db, 'accounts', acc.id, 'birthdayRequests'));
...
export async function approveBirthdayRequest(uid, profileId, birthday) {
  ...
  if (prof) {
    ...
    await updateDoc(doc(db, 'accounts', uid, 'profiles', profileId), { birthday, age });
  }
  await updateDoc(doc(db, 'accounts', uid, 'birthdayRequests', profileId), { status: 'approved' });
}

export async function rejectBirthdayRequest(uid, profileId) {
  await updateDoc(doc(db, 'accounts', uid, 'birthdayRequests', profileId), { status: 'rejected' });
}
...
export async function replyToFeedback(feedbackId, uid, replyText, adminName) {
  // Store reply on user's account so they get it on next load
  if (uid) {
    await setDoc(doc(db, 'accounts', uid, 'notifications', feedbackId), {
      type: 'feedback_reply',
      feedbackId,
      reply: replyText,
  ...
  try {
    await updateDoc(doc(db, 'accounts', uid, 'feedback', feedbackId), {
      reply: replyText,
