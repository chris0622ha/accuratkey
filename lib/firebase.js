...
export async function hasAnyProfileAdmin(uid) {
  const profiles = await getProfilesForAdmin(uid);
...
export async function adminSetProfileAdmin(targetUid, profileId, isProfileAdmin) {
  const ref = doc(db, 'accounts', targetUid, 'profiles', profileId);
...
export async function getFlaggedScores() {
  const q = query(collection(db, 'flaggedScores'), orderBy('createdAt', 'desc'), limit(100));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}

// Returns pending user-submitted restore requests, newest first.
export async function getRestoreRequests() {
  const q = query(collection(db, 'restoreRequests'), orderBy('createdAt', 'desc'), limit(100));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function approveFlaggedScore(flaggedScoreId, { type, uid, date, levelId }) {
  if (type === 'daily') {
    await updateDoc(doc(db, 'dailyScores', date, 'scores', uid), { removed: false });
  } else if (type === 'level') {
    await updateDoc(doc(db, 'leaderboard', String(levelId), 'scores', uid), { removed: false });
  }
  await updateDoc(doc(db, 'flaggedScores', flaggedScoreId), { status: 'approved', resolvedAt: serverTimestamp() });
}

// Admin dismisses a flagged score (confirms it should stay removed).
export async function dismissFlaggedScore(flaggedScoreId) {
  await updateDoc(doc(db, 'flaggedScores', flaggedScoreId), { status: 'dismissed', resolvedAt: serverTimestamp() });
}

// Admin marks a user's restore request as handled.
export async function resolveRestoreRequest(requestId, status) {
  await updateDoc(doc(db, 'restoreRequests', requestId), { status, resolvedAt: serverTimestamp() });
}
