...
import {
  ...
  collection, collectionGroup, query, orderBy, limit, getDocs, deleteDoc,
...
export async function deleteProfile(uid, profileId) {
  // Previously only deleted the profile document itself, leaving its
  // sessions subcollection (every WPM/accuracy/level-attempt record) sitting
  // in Firestore forever with no parent pointer to it — that's not real
  // deletion under COPPA's definition, which requires data to be removed
  // such that it "cannot be retrieved in the normal course of business."
  // An orphaned subcollection is still retrievable by anyone who knows the
  // path, it's just no longer reachable from the app's own UI.
  const sessionsSnap = await getDocs(collection(db, 'accounts', uid, 'profiles', profileId, 'sessions'));
  if (!sessionsSnap.empty) {
    const batch = writeBatch(db);
    sessionsSnap.docs.forEach(d => batch.delete(d.ref));
    await batch.commit();
  }
  // Note: dailyScores/{date}/scores/{uid} entries are keyed by account uid
  // (not profile id) and partitioned per calendar date, so there's no fixed
  // path to clean up here without scanning every date the account ever
  // played — those entries already age out naturally and restricted
  // profiles are blocked from creating new ones (see submitDailyScore call
  // site in TypingGame.jsx), so this is a known, bounded gap rather than an
  // ongoing one.
...
// ─── Leaderboard ──────────────────────────────────────────────────────────────
// (submitLeaderboardEntry and getLeaderboard, an older per-level leaderboard
// system that used to write to leaderboard/{levelId}/scores/{uid}, were
// removed here — zero call sites anywhere in the app, and
// submitLeaderboardEntry wrote a name/avatar/uid to a public leaderboard
// with no isProfileRestricted() check. The live per-day leaderboard system
// is submitDailyScore/getDailyLeaderboard above, which is correctly gated.)
...
// COPPA pre-existing-data audit: scans every account's profiles and returns
// counts only — no names, emails, UIDs, or any other identifying field ever
// leaves this function. Used by the one-time admin audit tool to answer
// "how many existing profiles are already restricted" without anyone,
// including the admin running it, browsing individual records to find out.
export async function getCoppaAuditSummary(firestoreDb = db) {
  const snap = await getDocs(collection(firestoreDb, 'accounts'));
  let totalAccounts = 0;
  let totalProfiles = 0;
  let restrictedProfiles = 0;
  let restrictedWithPhoto = 0;
  let restrictedWithPublicUsername = 0;
  let restrictedWithSessions = 0;

  for (const d of snap.docs) {
    totalAccounts++;
    const hasUsername = !!d.data().username;
    const profiles = await getDocs(collection(firestoreDb, 'accounts', d.id, 'profiles'));
    for (const p of profiles.docs) {
      totalProfiles++;
      const data = p.data();
      if (isProfileRestricted(data)) {
        restrictedProfiles++;
        if (data.photoURL) restrictedWithPhoto++;
        if (hasUsername) restrictedWithPublicUsername++;
        // Just check whether the sessions subcollection is non-empty,
        // don't read its contents.
        const sessionsSnap = await getDocs(collection(firestoreDb, 'accounts', d.id, 'profiles', p.id, 'sessions'));
        if (!sessionsSnap.empty) restrictedWithSessions++;
      }
    }
  }
  ...
  return {
    ...
    restrictedProfiles,
    restrictedWithPhoto,
    restrictedWithPublicUsername,
    restrictedWithSessions,
    scannedAt: new Date().toISOString(),
  };
}

// COPPA pre-existing-data cleanup: fixes what the audit above finds.
//  1. Strips photoURL from any restricted profile's own document.
//  2. Deletes that profile's entire sessions subcollection.
//  3. Deletes dailyScores entries — but ONLY for accounts where every single
//     profile on the account is restricted. dailyScores is keyed by account
//     uid, not profile id, so a score can't be attributed to one specific
//     profile when an account has a mix of restricted and non-restricted
//     profiles — deleting it there would risk wiping a different, legitimate
//     profile's real score. When the WHOLE account is restricted, there is
//     no other profile it could belong to, so this is safe. Mixed accounts
//     are skipped and counted separately so they're visible rather than
//     silently ignored.
//
// NOT automated:
//  - "public username" exposure — already handled by getPublicProfileFull()
//    excluding restricted profiles from the public page entirely; the
//    username belongs to the ACCOUNT and may still be needed by a different,
//    non-restricted profile on the same account.
export async function runCoppaCleanup(firestoreDb = db) {
  const snap = await getDocs(collection(firestoreDb, 'accounts'));
  let profilesCleaned = 0;
  let photosRemoved = 0;
  let sessionsDeleted = 0;
  let fullyRestrictedAccountUids = [];
  let mixedAccountsSkipped = 0;

  for (const d of snap.docs) {
    const profiles = await getDocs(collection(firestoreDb, 'accounts', d.id, 'profiles'));
    if (profiles.empty) continue;
    const profileDataList = profiles.docs.map(p => ({ doc: p, data: p.data() }));
    const allRestricted = profileDataList.every(p => isProfileRestricted(p.data));
    const anyRestricted = profileDataList.some(p => isProfileRestricted(p.data));
    if (anyRestricted && !allRestricted) mixedAccountsSkipped++;
    if (allRestricted) fullyRestrictedAccountUids.push(d.id);

    for (const { doc: p, data } of profileDataList) {
      if (!isProfileRestricted(data)) continue;

      let touched = false;

      if (data.photoURL) {
        await updateDoc(doc(firestoreDb, 'accounts', d.id, 'profiles', p.id), { photoURL: null });
        photosRemoved++;
        touched = true;
      }
      ...
      if (!sessionsSnap.empty) {
        ...
        await batch.commit();
        sessionsDeleted += sessionsSnap.docs.length;
        touched = true;
      }

      if (touched) profilesCleaned++;
    }
  }

  // Now handle dailyScores for fully-restricted accounts only. A collection
  // group query on "scores" returns every dailyScores/{date}/scores/{uid}
  // doc (and every leaderboard/{levelId}/scores/{uid} doc too, since both
  // use a subcollection literally named "scores") in one pass, regardless
  // of how many date documents exist — there's no way to list the date
  // documents directly, so this is the only way to reach them all.
  let dailyScoresDeleted = 0;
  if (fullyRestrictedAccountUids.length > 0) {
    const restrictedSet = new Set(fullyRestrictedAccountUids);
    const scoresSnap = await getDocs(collectionGroup(firestoreDb, 'scores'));
    const toDelete = scoresSnap.docs.filter(s => {
      // Only touch dailyScores/{date}/scores/{uid}, not the unrelated
      // leaderboard/{levelId}/scores/{uid} structure (which has no live
      // call sites and isn't where the real exposure is) or any other
      // future use of a "scores" subcollection name.
      const path = s.ref.path;
      return path.startsWith('dailyScores/') && restrictedSet.has(s.id);
    });
    if (toDelete.length > 0) {
      // Firestore batches cap at 500 writes; chunk defensively.
      for (let i = 0; i < toDelete.length; i += 450) {
        const batch = writeBatch(firestoreDb);
        toDelete.slice(i, i + 450).forEach(s => batch.delete(s.ref));
        await batch.commit();
      }
      dailyScoresDeleted = toDelete.length;
    }
  }
...
// ─── Maintenance mode ─────────────────────────────────────────────────────────
export async function setMaintenanceMode(enabled, message = '', triggers = {admins:false,users:true}, firestoreDb = db) {
...
// ─── Public profile ────────────────────────────────────────────────────────────
// (getPublicProfile, the older/simpler version of this, was removed here —
// it had no isProfileRestricted() check at all and returned every profile's
// name/avatar/level/WPM/streak unconditionally. It had zero call sites
// anywhere in the app, but dead code with a real exposure is still a risk if
// anyone ever wires it back in without knowing why getPublicProfileFull
// exists instead. Use getPublicProfileFull below for anything public-facing.)

// ─── Shop / themes ─────────────────────────────────────────────────────────────
// Authoritative price list - the ONLY place a theme's real cost is defined.
// purchaseTheme() looks the cost up here instead of trusting whatever value
// the caller passes in, because that value previously came straight from a
// plain client-side array in TypingGame.jsx with no server-side mirror to
// check against - meaning anyone could call purchaseTheme(uid, profileId,
// "neon", 0) directly from the browser console and get a "100 Keys" theme
// for free, since the function only checked "do you have enough for
// whatever cost you claim," never "is this the real price."
// Authoritative price list - matches the ORIGINAL shop design (22 themes,
// see components/ShopPage.jsx for the full visual definitions). The
// previous version of this list only had 8 themes with different,
// invented prices, built without realizing the real shop already existed
// with its own real pricing.
...
export async function purchaseTheme(uid, profileId, themeId) {
  const cost = THEME_PRICES[themeId];
  if (cost === undefined) throw new Error('Unknown theme');
  const ref = doc(db, 'accounts', uid, 'profiles', profileId);
  const snap = await getDoc(ref);
  if (!snap.exists()) throw new Error('Profile not found');
  const data = snap.data();
...
// Authoritative price list - matches the ORIGINAL shop design (30 real
// Google Fonts, see components/ShopPage.jsx for the full font list). The
// previous version had 55 fonts at invented prices, expanding far beyond
// what was actually designed, again without realizing the real shop and
// its real pricing already existed.
export const FONT_PRICES = {
  jetbrains: 0,
  firacode: 50,
  sourcecodepro: 50,
  robototmono: 50,
...
export async function purchaseFont(uid, profileId, fontId) {
  const cost = FONT_PRICES[fontId];
  if (cost === undefined) throw new Error('Unknown font');
  const ref = doc(db, 'accounts', uid, 'profiles', profileId);
  const snap = await getDoc(ref);
  if (!snap.exists()) throw new Error('Profile not found');
  const data = snap.data();
...
export const SOUND_PRICES = {
  default: 0, mechanical: 30, typewriter: 30, soft: 20, arcade: 40, nature: 50,
};

export async function purchaseSound(uid, profileId, soundId) {
  const cost = SOUND_PRICES[soundId];
  if (cost === undefined) throw new Error('Unknown sound pack');
  const ref = doc(db, 'accounts', uid, 'profiles', profileId);
  const snap = await getDoc(ref);
  if (!snap.exists()) throw new Error('Profile not found');
  const data = snap.data();
  if ((data.keys || 0) < cost) throw new Error('Not enough keys');
  const owned = data.ownedSounds || [];
  if (owned.includes(soundId)) throw new Error('Already owned');
  await updateDoc(ref, { keys: (data.keys || 0) - cost, ownedSounds: [...owned, soundId] });
}

export async function setActiveSound(uid, profileId, soundId) {
  await updateDoc(doc(db, 'accounts', uid, 'profiles', profileId), { activeSound: soundId });
}

// ─── Challenge a friend ────────────────────────────────────────────────────────
// (sendChallenge and getChallenges, an older/simpler version of the
// challenge system, were removed here — zero call sites; superseded by
// sendChallengeEx/getPendingChallenges, which the live Challenges modal
// uses and which is already excluded entirely for restricted profiles.)
  ...
  for (const acc of accounts.docs) {
    ...
    for (const p of profiles.docs) {
      ...
      sessions.docs.forEach(s => {
        const d = s.data();
        // Skip sessions with no level field, or with non-numeric level values
        // (-1 is the daily challenge sentinel, undefined means a corrupt/legacy
        // session doc, both were showing as "Level -1" and "Level undefined"
        // in the analytics display)
        const lvl = d.level;
        if (lvl == null || typeof lvl !== 'number') return;
        if (!d.passed) {
          const key = lvl === -1 ? 'Daily Challenge' : String(lvl);
...
export async function getAdminFeedback(limitN = 50, firestoreDb = db) {
  ...
  // Also include truly anonymous feedback (submitted with no signed-in uid,
  // stored in the root 'feedback' collection by submitFeedback()). This was
  // previously invisible in the admin panel entirely — not just unreplyable,
  // but never shown at all. These can't receive a reply since there's no
  // account to attach a notification to, but admins should at least see them.
  try {
    const anonSnap = await getDocs(query(
    ...
    anonSnap.docs.forEach(d => {
      const data = d.data();
      all.push({
        id: d.id, uid: null, text: data.text,
        profileId: data.profileId,
        username: data.username || null,
        profileName: data.profileName || null,
        screenshot: data.screenshot || null,
  ...
  } catch(e) { /* skip if no root feedback collection */ }
  // Sort by date desc
...
export async function sendChallengeEx(fromUid, fromUsername, fromAvatar, toUid, toUsername, levelId, levelName, gameMode = null) {
  await addDoc(collection(db, 'challenges'), {
    fromUid, fromUsername, fromAvatar,
    toUid, toUsername,
    levelId, levelName,
    gameMode, // null = level challenge (existing behavior). "tugofwar" etc. = a head-to-head game challenge instead.
    sentAt: serverTimestamp(),
    status: 'pending', // pending | accepted | declined | completed
    fromResult: null,  // {wpm, accuracy, passed}
    toResult: null,
    // Live shared state for head-to-head games - both players write their
    // own rope position here and read the opponent's via onSnapshot, so
    // the match feels real-time without needing a separate database.
    liveState: null, // {fromRope, toRope, winnerUid, startedAt}
  });
}
...
// Updates this player's rope position in the shared challenge doc. Whichever
// side calls this writes only their own field (fromRope/toRope depending on
// which side they are) - both players read the FULL doc via a live
// subscription so each sees the other's position update in real time.
export async function updateGameLiveState(challengeId, isFromSide, ropeValue) {
  const field = isFromSide ? 'liveState.fromRope' : 'liveState.toRope';
  await updateDoc(doc(db, 'challenges', challengeId), { [field]: ropeValue });
}

export async function startGameChallenge(challengeId) {
  await updateDoc(doc(db, 'challenges', challengeId), {
    status: 'accepted',
    liveState: { fromRope: 0, toRope: 0, winnerUid: null, startedAt: serverTimestamp() },
  });
}

export async function setGameWinner(challengeId, winnerUid) {
  await updateDoc(doc(db, 'challenges', challengeId), {
    status: 'completed',
    'liveState.winnerUid': winnerUid,
  });
}

// Live subscription to a single challenge doc - used by both players during
// an active head-to-head match so each sees the other's rope position
...
export async function replyToFeedback(feedbackId, uid, replyText, adminName, firestoreDb = db) {
  // Notifications stored at top-level /pendingNotifications/{uid}/{notifId}
  // instead of /accounts/{uid}/notifications/ because Firestore rules
  // prevent the admin from writing to another user's own subcollection.
  // The top-level collection can be opened to any authenticated write
  // without needing special admin-impersonation rules.
  if (uid) {
    await setDoc(doc(firestoreDb, 'pendingNotifications', uid, 'items', feedbackId), {
      type: 'feedback_reply',
      feedbackId,
      reply: replyText,
  ...
  // Mark the feedback doc itself as replied so the admin panel
  // shows the reply status correctly.
...
export async function getPendingNotifications(uid) {
  ...
  const snap = await getDocs(
    query(collection(db, 'pendingNotifications', uid, 'items'),
    where('read', '==', false))
  );
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
...
export async function markNotificationRead(uid, notifId) {
  if (!uid) return;
  await updateDoc(doc(db, 'pendingNotifications', uid, 'items', notifId), { read: true });
}
