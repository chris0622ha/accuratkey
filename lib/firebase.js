...
export async function submitDailyScore(uid, username, avatar, { wpm, accuracy }) {
  ...
  if (suspicious) {
    await addDoc(collection(db, 'flaggedScores'), { uid, username, wpm, accuracy, date: today, reason: `WPM ${wpm} exceeds maximum`, status: 'pending', createdAt: serverTimestamp() });
  }
  return { suspicious };
}
...
export async function requestScoreRestore(uid, date, reason) {
  await addDoc(collection(db, 'restoreRequests'), { uid, date, reason, status: 'pending', createdAt: serverTimestamp() });
}
