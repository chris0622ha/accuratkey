...
export async function replyToFeedback(feedbackId, uid, replyText, adminName, firestoreDb = db) {
  if (uid) {
    // Use the main authenticated db (not adminDb) for this write because
    // the Firestore rules require auth != null, and adminDb is a separate
    // unauthenticated app instance that fails auth checks. The admin user
    // IS authenticated in the browser via the regular db instance.
    await setDoc(doc(db, 'pendingNotifications', uid, 'items', feedbackId), {
      type: 'feedback_reply',
      feedbackId,
      reply: replyText,
  ...
  // Mark the feedback doc itself as replied - adminDb is fine here since
  // it reads/writes the accounts collection which has different rules.
