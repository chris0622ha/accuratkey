...
export async function replyToFeedback(feedbackId, uid, replyText, adminName, firestoreDb = db) {
  ...
  // Also mark feedback as replied. Previously wrapped in a try/catch that
  // swallowed every error silently — if this write failed (e.g. a security
  // rule rejecting the admin app's write to a user's subcollection), the
  // admin UI still showed "Send Reply" as successful and updated its local
  // list optimistically, but the reply was never actually persisted to
  // Firestore — so it would vanish again on refresh. Letting this throw lets
  // the caller's existing catch(e){ alert(...) } actually do its job.
