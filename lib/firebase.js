...
// COPPA pre-existing-data cleanup: actually fixes what the audit above finds,
// for the two things that are safe to automate without ambiguity:
//  1. Strips photoURL from any restricted profile's own document.
//  2. Deletes that profile's entire sessions subcollection.
// Both are scoped to exactly one profile's own data, so there's no risk of
// touching a different (non-restricted) profile's history.
//
// NOT automated here, on purpose:
//  - "public username" exposure — already handled by getPublicProfileFull()
//    excluding restricted profiles from the public page entirely (verified
//    still in place above); the username itself belongs to the ACCOUNT, not
//    the specific profile, and may still be legitimately needed for another
//    non-restricted profile on the same account. Removing it could break a
//    parent's own public page over an unrelated child profile.
//  - dailyScores entries — keyed by account uid, not profile id, so a score
//    can't be attributed to a specific profile with certainty. Deleting by
//    uid risks removing a DIFFERENT, non-restricted profile's legitimate
//    score if they share the account. Left as a documented limitation
//    rather than risk deleting the wrong person's data.
