...
// Help-doc body text is plain strings, not JSX, so the real KKey icon can't
// just be dropped in like {body}. This splits on a literal "{KKEY}" token
// (used in the content below instead of the 🔑 emoji) and renders the real
// icon component between the surrounding text segments.
function withKKeyIcon(text: string) {
  const parts = text.split('{KKEY}');
  if (parts.length === 1) return text;
  const out: React.ReactNode[] = [];
  parts.forEach((part, i) => {
    out.push(part);
    if (i < parts.length - 1) out.push(<KKey key={i} size={12} />);
  });
  return out;
}
...
const TOPICS: Record<string, Topic> = {
  ...
  'getting-started': {
    ...
    faqs: [
      ['Do I need an account?', 'No. Guest mode gives full access to all 180 levels and all 34 games. Progress saves in your browser. Sign up to sync across devices, use friends and daily challenge features.'],
  ...
  'levels': {
    ...
    sections: [
      ...
      { heading: 'Keys earned', body: 'Completing a level earns {KKEY} Keys. Higher levels pay more. A combo multiplier applies if you type 10+ words correctly in a row (1.5×) or 20+ (2×).' },
    ],
    faqs: [
  ...
  'games': {
    ...
    sections: [
      ...
      { heading: '📚 Educational Games', body: 'Vocab Builder, Synonyms, Antonyms. Learn vocabulary while typing.' },
    ...
    faqs: [
      ...
      ['Are games in guest mode?', 'Yes — all 34 games are fully playable as a guest.'],
  ...
  'keys-shop': {
    ...
    intro: '{KKEY} Keys are AccuratKey\'s in-game currency. Earn them by completing levels, spend them on cosmetics.',
    sections: [
      ...
      { heading: 'Gifting Keys', body: 'If the Send Keys feature is enabled on your profile, you can gift Keys to friends. Open a friend\'s profile from the Friends panel and tap Gift {KKEY}.' },
  ...
  'daily': {
    ...
    sections: [
      { heading: 'How it works', body: 'Every day at midnight Eastern Time a new word set is generated. Every player types the exact same words. Your score (WPM × accuracy ÷ 100) goes on the leaderboard. Only your first attempt counts — no retries.' },
      { heading: 'Streaks 🔥', body: 'Your streak counts consecutive days played — complete at least one typing level or the daily challenge each calendar day (Eastern Time) to keep it alive. Miss a day and the streak resets to 0. Your current streak shows in the nav bar.' },
      { heading: 'Daily Leaderboard', body: 'After completing, tap the Daily tab to see today\'s global rankings. The leaderboard updates in real time. Rankings reset every day at midnight Eastern Time.' },
      { heading: 'Availability', body: 'Daily challenge requires a logged-in account. Guest mode does not have access. The challenge becomes available again each day at midnight Eastern Time, regardless of your own timezone.' },
    ...
    faqs: [
      ...
      ['What time does it reset?', 'Midnight Eastern Time (America/New_York), regardless of your own timezone.'],
  ...
  'streaks': {
    ...
    sections: [
      { heading: 'Streaks 🔥', body: 'Your streak counts consecutive days played. Complete at least one typing level or the daily challenge each calendar day (Eastern Time) to maintain it. The streak count shows in the nav bar. Missing a single day resets your streak to 0. There is no grace period.' },
    ...
    faqs: [
      ['Does streak reset at midnight exactly?', 'Yes — midnight Eastern Time (America/New_York), not your local timezone.'],
  ...
  'troubleshooting': {
    ...
    sections: [
      ...
      { heading: 'Streak reset unexpectedly', body: 'Streaks reset based on the calendar day in Eastern Time (America/New_York), not your device\'s local timezone. If you completed a level very close to midnight Eastern Time, it may have counted for the day before or after what you expected.' },
    ],
    faqs: [
  ...
  'faq': {
    ...
    faqs: [
      ...
      ['How do streaks work?', 'Your streak counts consecutive days played. Complete one level or the daily challenge each calendar day (Eastern Time) to keep it going. Miss a day and it resets to 0. Streaks show in the nav bar as 🔥N.'],
      ...
      ['What are Keys {KKEY}?', 'The in-game currency. Earned by completing typing levels. Spent in the Shop on themes, fonts, and sound packs. Can be gifted to friends. No real-money involvement.'],
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0a0a0f',fontFamily:"'JetBrains Mono',monospace",color:'#e0e0ff',padding:'0 20px'}}>
      ...
      <main style={{maxWidth:740,margin:'40px auto 80px'}}>
        ...
        <p style={{color:'#666',fontSize:14,lineHeight:1.7,marginTop:16,marginBottom:32}}>{withKKeyIcon(intro)}</p>
        ...
        {sections.map(({heading,body}) => (
          <div key={heading} style={{background:'#13131f',border:'1px solid #1e1e30',borderRadius:12,padding:'18px 22px',marginBottom:10}}>
            ...
            <p style={{color:'#888',fontSize:13,lineHeight:1.8,margin:0}}>{withKKeyIcon(body)}</p>
          </div>
        ))}
            ...
            {faqs.map(([q,a],i) => (
              <div key={i} style={{background:'#13131f',border:`1px solid ${openFaq===i?color+'66':'#1e1e30'}`,borderRadius:10,marginBottom:6,overflow:'hidden'}}>
                <button onClick={()=>setOpenFaq(openFaq===i?null:i)} style={{width:'100%',padding:'13px 18px',background:'none',border:'none',color:'#e0e0ff',fontFamily:"'JetBrains Mono',monospace",fontSize:13,fontWeight:700,cursor:'pointer',textAlign:'left',display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}>
                  <span>{withKKeyIcon(q)}</span>
                  <span style={{color,fontSize:16,flexShrink:0}}>{openFaq===i?'−':'+'}</span>
                </button>
                {openFaq===i && <div style={{padding:'0 18px 14px',color:'#777',fontSize:13,lineHeight:1.8}}>{withKKeyIcon(a)}</div>}
              </div>
            ))}
          </div>
        )}
