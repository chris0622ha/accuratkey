...
const TypingTest = ({ T, customWords }) => {
  ...
  const mkWords = (pool) => {
    const src = (pool && pool.length > 0) ? pool : DEFAULT_WORDS;
    let s = "", a = [];
    while (s.length < 300) { const w = src[Math.floor(Math.random() * src.length)]; a.push(w); s += w + " "; }
    return a;
  };

  const [words, setWords] = useState(() => mkWords(customWords));
  ...
  // Reset when customWords changes
  const prevPool = useRef(customWords);
  if (prevPool.current !== customWords) {
    prevPool.current = customWords;
    // Can't call setState during render in strict sense, but useEffect would delay;
    // instead we track via ref and reset on next interaction
  }
  ...
  const reset = (pool) => {
    ...
    setWords(mkWords(pool !== undefined ? pool : customWords));
    setTyped(""); setStart(null); setDone(false); setWpm(0); setAcc(100);
