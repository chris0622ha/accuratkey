...
export default function AccuratKey() {
  ...
  const selectProfile = async (profile) => {
    profileRoutedRef.current = true; // Prevent onAuthStateChanged race condition
            ...
            if (playingLevel === -1) {
              submitDailyScore(user.uid, currentUsername, activeProfile.avatar, {wpm:fw, accuracy:newAcc}).then(result => {
                if (result?.suspicious) {
                  setTimeout(() => {
                    if (window.confirm(`Your score of ${Math.round(fw)} WPM was flagged and removed from the leaderboard.\n\nTap OK to request a review.`)) {
                      requestScoreRestore(user.uid, null, `User claims ${Math.round(fw)} WPM is legitimate`).catch(()=>{});
                      window.alert("Review requested!");
                    }
                  }, 1500);
                }
              }).catch(()=>{});
        ...
        {profiles.map(p => (
          ...
          >
            <div onClick={() => selectProfile(p)}
              ...
              <div style={{color:T.accent,fontSize:11,marginTop:2,display:"flex",alignItems:"center",justifyContent:"center",gap:3}}><KKey size={11}/>{((k)=>{const n=Math.round(k);return n>=1e6?""+Math.round(n/1e6)+"M":n>=1e3?""+Math.round(n/1e3)+"k":n})(p.keys||0)}</div>
            </div>
                  ...
                  <div style={{marginTop:8}}>
                    <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",color:T.faint,fontSize:12}}>
                      <input type="file" accept="image/*" style={{display:"none"}} onChange={e=>{
                        ...
                        const r=new FileReader();r.onload=ev=>setFeedbackScreenshot(ev.target?.result);r.readAsDataURL(f);
                      }}/>
