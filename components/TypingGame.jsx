...
// Sound
const playSound=(type)=>{try{const ctx=new(window.AudioContext||window.webkitAudioContext)();const o=ctx.createOscillator(),g=ctx.createGain();o.connect(g);g.connect(ctx.destination);if(type==="correct"){o.frequency.value=880;g.gain.setValueAtTime(0.08,ctx.currentTime);g.gain.exponentialRampToValueAtTime(0.001,ctx.currentTime+0.07);o.start();o.stop(ctx.currentTime+0.07);}else if(type==="wrong"){o.type="sawtooth";o.frequency.value=160;g.gain.setValueAtTime(0.12,ctx.currentTime);g.gain.exponentialRampToValueAtTime(0.001,ctx.currentTime+0.1);o.start();o.stop(ctx.currentTime+0.1);}else if(type==="complete"){const t=ctx.currentTime;[523,659,784].forEach((f,i)=>{const o2=ctx.createOscillator();o2.connect(g);o2.frequency.value=f;o2.start(t+i*0.1);o2.stop(t+i*0.1+0.15);});}}catch(e){}};
    ...
    return (
      <div onClick={() => inputRef.current?.focus()} style={{minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column",alignItems:"center",padding:"20px 12px",fontFamily:T.font,userSelect:"none"}}>
        ...
        <div style={{width:"100%",maxWidth:660,background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"20px 24px",marginBottom:12}}>
          {lines[lineIdx+1] && <div style={{fontSize:14,letterSpacing:1,marginBottom:10,color:"#2a2a3a",overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>{lines[lineIdx+1]}</div>}
          <div style={{fontSize:20,letterSpacing:2,display:"flex",flexWrap:"wrap",lineHeight:1.8}}>
